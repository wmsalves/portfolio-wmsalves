...
function levelBadge(level: SkillItem["level"]) {
  ...
  if (level === "familiar") {
    return (
      <span
        className="ml-2 rounded-full border border-gray-300 text-gray-700 text-[10px] font-medium px-2 py-0.5
  ...
  return (
    <span
      className="ml-2 rounded-full border border-amber-300 bg-amber-50 text-amber-800 text-[10px] font-semibold px-2 py-0.5
    ...
    <div>
      ...
      <ul className="grid grid-cols-2 gap-4">
        {items.map((item) => (
          <li
            key={item.name}
            className="rounded-lg border border-gray-200 bg-white p-5 sm:p-6 text-center shadow-sm
          ...
          >
            <span className="text-sm font-medium text-gray-900 dark:text-gray-100 align-middle">
              {item.name}
            </span>
    ...
    >
      ...
      >
        <div className="space-y-10 sm:space-y-12 text-center">
          ...
          {/* Texto explicativo */}
          <div className="max-w-3xl mx-auto text-gray-700 dark:text-gray-300 leading-relaxed">
            <p>
